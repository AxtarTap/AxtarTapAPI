...
import {
    WorkerModel,
    getUserByAccessToken as getWorkerByAccessToken,
} from "../../models/workers";
import { RequestIdentity } from "../../types/types";
import {
    CustomerModel,
    getUserByAccessToken as getCustomerByAccessToken,
...
export const getUserInformation = async (req: Request, res: Response) => {
    try {
        ...
        const model: typeof CustomerModel | typeof WorkerModel =
            identity.type === 0 ? CustomerModel : WorkerModel;
        const userData = await model
            .findById(identity.user._id.toString())
            .select("-__v");
        const subsciptionData = await getSubscriptionByCustomerId(
        ...
        return res
            ...
            .json({
                type: identity.type,
                user: { userData, subscription: subsciptionData || null },
            })
            .end();
    } catch (err) {
...
export const refreshToken = async (req: Request, res: Response) => {
    try {
        ...
        const customer = await getCustomerByAccessToken(
            authToken,
            "+authentication.accessToken +authentication.refreshToken -__v"
        );
        const worker = await getWorkerByAccessToken(
            authToken,
            "+authentication.accessToken +authentication.refreshToken -__v"
        );
        let user = null;
        ...
        if (
            !refreshToken ||
            refreshToken.token !== user.authentication.refreshToken ||
            user.authentication.refreshToken !== refreshTokenCookie
        ) {
            return res.status(401).end();
        }
    ...
    try {
        ...
        if (!old_password) {
            errorHandler.addError(
                new APIError("system", "payload", "MISSING_PROPERTY"),
                { p: "old password" }
            );
        }
        ...
        if (old_password == new_password) {
            return errorHandler.handleError(
                new APIError("system", "payload", "INVALID_PAYLOAD"),
                { m: "old password and new password cannot be the same" }
            );
        }

        if (
            !(await passwordMatches(old_password, user.authentication.password))
        ) {
            return errorHandler.handleError(
                new APIError("system", "payload", "INCORRECT_PROPERTY"),
                { p: "Old password" }
            );
        }
