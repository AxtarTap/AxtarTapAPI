...
export const login = async (req: Request, res: Response) => {
    try {
        ...
        const user = await WorkerModel.findOne({ email }).select(
        ...
        if (user.authType !== 0) {
            return errorHandler.handleError(
                new APIError("registration", "email", "EMAIL_LINKED_TO_GOOGLE")
            );
        }
...
export const register = async (req: Request, res: Response) => {
    try {
        ...
        if (!username) {
            errorHandler.addError(
                new APIError("registration", "username", "MISSING_USERNAME")
            );
        }
        ...
        if (
            ...
            validateUsername(username, errorHandler)
        ) {
